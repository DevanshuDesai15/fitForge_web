...
class AIProviderService {
  constructor(config = aiProviderConfig, provider = huggingFaceService) {
  ...
  async generateProgressionSuggestions(analysisData, userProfile, workoutHistory) {
    try {
      if (this.disabled || !this.config.useAIProvider) {
...
export const getAIProviderModel = () => null;
...
export const generateWorkoutAnalysis = async (
  ...
  return aiProviderService.generateWorkoutAnalysis(exercise, completedSets);
};
